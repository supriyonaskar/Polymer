...
#5. Chain length distribution it should be written like x1 x2 x3 x4. Suppose you want to build a polymer with chains havinf 10-mer, 15-mer and 20-mer then write 10 15 20  (Not necessary for v2)\n
#6. Name of the molecule (You should remember that your initial xyz file of the monomer should have a name like {moleculename}_initial.xyz), also make sure xyz file have two remarks like in begining. \n 
#7. Flag to keep or remove all the unnecessary files dusing the code execution. if 0 it keeps everything, elseif 1 it removes everything \n
#8. Flag to optimize or not, if 1 it will optimize, if 0 it will build the polymer...else not 
    ...
    if not li.startswith("#"):
        if len(line.rstrip()):
            if  i == 0: 
                ...
                chains  = (line.rstrip())
                chains = np.array(str.split(chains))
                i = i+1
            elif  i == 5 : 
