    ...
    if not li.startswith("#"):
        if len(line.rstrip()):
            if  i == 0: 
                ...
                head2  = int(line.rstrip())
                i = i+1
            elif  i == 8:
                tail2  = int(line.rstrip())
                i = i+1
            elif  i == 9:
                head2_H  = int(line.rstrip())
                i = i+1
            elif  i == 10:
                tail2_H  = int(line.rstrip())
                i = i+1
            elif  i == 11:
                molecule_name2 = str(line.rstrip())
                i =i+1
#print(head1,tail1,head1_H,tail1_H,molecule_name,remove1,opt,head2,tail2,head2_H,tail2_H,molecule_name2)
...
if opt ==0:
    name1 = molecule_name
    name2 = molecule_name2
    def monomer1(is_capped=True):
        s = system.read_mol2(F'{molecule_name}_monomer.mol2')
        m = s.molecules[1]
        amber.get_forcefield_types_cop(s, types='gaff', f=forcefield.Gaff(), name=name1)
    ...
    pe1  = monomer1(is_capped=False)

    def monomer2(is_capped=True):
        s = system.read_mol2(F'{molecule_name2}_monomer.mol2')
        m = s.molecules[1]
        amber.get_forcefield_types_cop(s, types='gaff',f=forcefield.Gaff(),name=name2)
        for b in s.bonds:
            if b.a.bonds.count == 4 and b.b.bonds.count == 4:
                b.order = 5
        s.apply_forcefield(forcefield.Gaff())
        setattr(s.particles[head2_H], 'rnd_wlk_tag', 'head_cap')
        setattr(s.particles[tail2_H], 'rnd_wlk_tag', 'tail_cap')
        c1 = s.particles[head2]
        c2 = s.particles[tail2]
        c1.linker = 'head'
        c2.linker = 'tail'
    ...
    for i in range (10,51):
        ...
        for j in range (i+1):
            if random.random() < 0.5 :
                ...
                list1.append("pe2") 
            list2 = ', '.join(list1)
        exec(f'polymer_{i} = copolymer([{list2}], {i}, forcefield=forcefield.Gaff(), density = 0.001)')
        exec(f'polymer_{i}.center(what="particles", at=[0.0, 0.0, 0.0], move_both=False)')
        print ("polymerlength=",i,list2)
