...
for line in open("input_parameters.dat"):
    ...
    if not li.startswith("#"):
        if len(line.rstrip()):
            if  i == 0: 
                head111  = (line.rstrip())
                head111  = np.array(str.split(head111))
                head11   = int(head111[0])
                head12   = int(head111[1])
                i = i+1
            elif  i == 1:
                tail111  = (line.rstrip())
                tail111  = np.array(str.split(tail111))
                tail11   = int(tail111[0])
                tail12   = int(tail111[1])
                i = i+1
            elif  i == 2:
                head111_H  = (line.rstrip())
                head111_H  = np.array(str.split(head111_H))
                head11_H   = int(head111_H[0])
                head12_H   = int(head111_H[1])
                i = i+1
            elif  i == 3:
                tail111_H  = (line.rstrip())
                tail111_H  = np.array(str.split(tail111_H))
                tail11_H   = int(tail111_H[0])
                tail12_H   = int(tail111_H[1])
                i = i+1
            elif  i == 4 : 
...
if opt ==1:
    ...
    options = {'constraint_charge': [[0.0, [head11_H]], [0.0, [head12_H]], [0.0, [tail11_H]], [0.0, [tail12_H]]]}
...
if opt == 0:
    def monomer1(is_capped=True):
        ...
        #amber.calc_charges(s, charge_method='bcc', cleanup=False)
        amber.get_forcefield_types(s, types='gaff',f=forcefield.Gaff())
        s.apply_forcefield(forcefield.Gaff())
        setattr(s.particles[head11_H], 'rnd_wlk_tag', 'head_cap')
        setattr(s.particles[head12_H], 'rnd_wlk_tag', 'head_cap')
        setattr(s.particles[tail11_H], 'rnd_wlk_tag', 'tail_cap')
        setattr(s.particles[tail12_H], 'rnd_wlk_tag', 'tail_cap')
        
        
    
        heads = [head11,head12]
        for t in heads:
            s.particles[t].linker = 'head'
            
        tails = [tail11,tail12]
        for t in tails:
            s.particles[t].linker = 'tail'
            
        c1 = s.particles[head11]
        c2 = s.particles[head12]
        c3 = s.particles[tail11]
        c4 = s.particles[tail12]
        ...
        #if not is_capped:
        #    for b in c1.bonds:
        #        if b.a.elem == 'H' or b.b.elem == 'H' or b.charge == 0.0:
        ...
        #            break
        #    for b in c3.bonds:
        #        if b.a.elem == 'H' or b.b.elem == 'H' and b.charge == 0.0:
        ...
        lmps.quick_min(s, min_style='fire', save_input=False, np=16)
    ...
    def cap_with_H(input_sst, ff):
        ...
        for p in input_sst.particles:
            if p.linker is not None:
                if p.linker == 'head' and len(p.bonded_to) == num_bond_H-1:
                ...
                elif p.linker == 'tail' and len(p.bonded_to) == num_bond_T-1:
        ...
        sim.add_min(min_style='cg', name='min_cg', etol=1.0e-8, ftol=1.0e-8, maxiter=int(1e+7), maxeval=int(1e+8))
        sim.run(np=16)
    ...
    for i in range (45,51):
        if i ==45:
            polymer_45 = random_walk(pe, 45, forcefield=forcefield.Gaff(),density = 0.001, extra_bonds=[1,0])
            polymer_45.write_yaml('polymer_45.yaml')
            exec('cap_with_H(polymer_45, forcefield.Gaff())')
            #lmps.quick_min(polymer_45, min_style='fire')
            sim1 = lmps.Simulation(polymer_45, log='steps.log')
            sim1.add_min(min_style='cg', name='minimization',  etol=1.0e-5, ftol=1.0e-5)
            sim1.add_md(ensemble='nvt', timestep=1, temperature=100, length =5000, new_v=True)
            sim1.run(np=16)
            polymer_45.write_lammps('polymer_45.lmps')
        else:
            indx = i-1
            exec(f'original_polymer = system.read_yaml("polymer_{indx}.yaml")')
            ln = 1            
            exec (f'polymer_{i} = copolymer([original_polymer, pe], 2, pattern=[1, 1], forcefield=forcefield.Gaff(),density = 0.001, extra_bonds=[1,0])')
