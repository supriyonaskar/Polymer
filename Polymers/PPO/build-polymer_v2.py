...
if opt ==1: 
    ...
    psi4.optimize('scf/cc-pVDZ', molecule=PBI, maxiter=500, geom_maxiter = 100)
...
if opt ==0:
    ...
    for i in range (30,51):
        if i ==30:
            polymer_30 = random_walk(pe, 30, forcefield=forcefield.Gaff(),density = 0.001)
            polymer_30.write_yaml('polymer_30.yaml')
            exec('cap_with_H(polymer_30, forcefield.Gaff())')
            lmps.quick_min(polymer_30, min_style="cg")
            polymer_30.write_lammps('polymer_30.lmps')
        else:
            indx = i-1
            ...
            exec(f'lmps.quick_min(polymer_{i}, min_style="cg")')
